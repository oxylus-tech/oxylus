...
class BundleRenderer:
    ...
    """
    Name to template file to render (as scoped by bundle's directory).
    """

    @cached_property
    def engine(self):
        """Engine to use in order to render templates."""
        return Engine(dirs=[self.bundle.get_template_dir()], apps_dir=False, libraries=self.template_libraries)

    def __init__(self, bundle: models.Bundle, libraries: dict[str, str] | None = None):
        self.bundle = bundle
        if libraries:
    ...
    def get_template_names(self):
        ...
        By default this method will return each string provided by :py:attr:`template_name` prefixed with bundle's slug.
        """
        if isinstance(self.template_name, str):
            return [f"{self.bundle.slug}/{self.template_name}"]
        return [f"{self.bundle.slug}/{template_name}" for template_name in self.template_name]
